CellPropPlot <- function(object, group.by = NULL, prop.in = NULL, 
                         cols = NULL){
    if(is.null(group.by)){
    ...
    p <- ggplot(data = df, aes_string(x = prop.in, y = proportion, fill = group.by)) +
...
CompareCellProp <- function(object, group.by = NULL, prop.in = NULL, sample.annotation = NULL,
                            cols = NULL, comparisons = NULL){
    if(is.null(group.by)){
        stop("Please specify how to group the cells!")
    }
    if(is.null(prop.in)){
    ...
    if(is.null(sample.annotation)){
        stop("Please specify the sample annotation which will used to group the sampels for statistical test!")
    }
    ...
    p <- ggplot(data = df, aes_string(x = sample.annotation, y = proportion)) +
...
TrajectoryPlot <- function(
  ...
  smoothWindow = 5
      ...
      df <- DataFrame(object@reductions[[reduction]]@cell.embeddings[, 1:2])
